 ...
 <div class ="div0">
  ...
  <table border=0 > <!-- 달력 상단 부분 -->
   <tr>
    ...
    <a href="reservedView?year=<%out.print(year-1);%>&month=<%out.print(month);%>&day=<%out.print(Integer.parseInt(request.getParameter("day"))); %>">◀</a>
    <% out.print(year); %>년
    <a href="reservedView?year=<%out.print(year+1);%>&month=<%out.print(month);%>&day=<%out.print(Integer.parseInt(request.getParameter("day"))); %>">▶</a>
    </td>
    <td align=center width=300> <!-- 월 -->
    <a href="reservedView?year=<%out.print(year);%>&month=<%out.print(month-1);%>&day=<%out.print(Integer.parseInt(request.getParameter("day"))); %>">◀</a>
    <% out.print(month+1); %>월
    <a href="reservedView?year=<%out.print(year);%>&month=<%out.print(month+1);%>&day=<%out.print(Integer.parseInt(request.getParameter("day"))); %>">▶</a>
    </td>
  				...
  				<td width=33%>
	  				<table border="0" cellspacing=0 cellpadding=0 width="100%" height="80%" >
		  				<c:forEach items="${rvdtos }" var="rvdto">
			  				...
			  				<tr ><td width=60% align="right">${rvdto.psspeciesname } :&nbsp; </td><td align="left" >
			  				${rvdto.puppycount }마리
			  				</td></tr> 					
			  				</c:if>
